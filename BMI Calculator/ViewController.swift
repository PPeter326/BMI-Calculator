...
class ViewController: UIViewController, UIPickerViewDataSource, UIPickerViewDelegate {
    ...
    var userInput: UserInput? {
        get {
            return UserInput(weightInLbs: weightInLbs, heightInInches: totalHeightInInches, weightInKgs: weightInKgs, totalHeightInCm: totalHeightCentimeters)
        }
        set {
...
//    var measurementSystem: MeasurementSystem = .imperial
    ...
    func loadUserInput() {
        ...
        if let fileURL = possibleFileURL {
            do {
                let weightData = try Data(contentsOf: fileURL)
                let jsonDecoder = JSONDecoder()
                // create user input from loaded weight data
                let input = try! jsonDecoder.decode(UserInput.self, from: weightData)
                userInput = UserInput(weightInLbs: input.weightInLbs, heightInInches: input.totalHeightInInches, weightInKgs: input.weightInKgs, totalHeightInCm: input.totalHeightInCm)
                print(userInput ?? "nil user data")
            } catch let error as NSError {
                print("error: \(error.domain) \(error.description)")
            }
        } else {
